...
import {
  getKnownVendorCodes,
  getVendorAlarmByCode,
  getVendorAlarmCatalog,
} from '../protocol/vendorAlarmCatalog'
...
export class JTT808Server {
  ...
  private latestResourceLists = new Map<
    string,
    { receivedAt: number; items: ResourceVideoItem[] }
  >()
  private pendingResourceLists = new Map<string, PendingResourceList>()
  private lastKnownLocation = new Map<
    string,
  ...
  private pendingScreenshotRequests = new Map<
    string,
  ...
  private readonly messageTraceEnabled = ['1', 'true', 'yes', 'on'].includes(
    String(
      process.env.MESSAGE_TRACE_ENABLED ??
        (process.env.ALERT_PROCESSING_ENABLED === 'false' &&
  ...
  private getVideoRequestProfile(): 'legacy' | 'main' {
    const raw = String(process.env.VIDEO_REQUEST_PROFILE || '')
      .trim()
      .toLowerCase()
  ...
  private getPreferredStreamType(): 0 | 1 {
    const explicit =
      process.env.VIDEO_REQUEST_STREAM_TYPE ?? process.env.VIDEO_STREAM_TYPE
    if (
      explicit !== undefined &&
      explicit !== null &&
  ...
  private getOptimizedVideoParameters(): {
    resolution: number
    frameRate: number
    bitrate: number
  } {
    ...
    return {
      resolution: Math.max(
        ...
        Math.min(
          4,
          Number(
            process.env.VIDEO_REQUEST_RESOLUTION ||
              process.env.VIDEO_RESOLUTION ||
              defaultResolution,
          ) || defaultResolution,
        ),
      ),
      frameRate: Math.max(
        ...
        Math.min(
          30,
          Number(
            process.env.VIDEO_REQUEST_FRAME_RATE ||
              process.env.VIDEO_FRAME_RATE ||
              defaultFrameRate,
          ) || defaultFrameRate,
        ),
      ),
      bitrate: Math.max(
        ...
        Math.min(
          8192,
          Number(
            process.env.VIDEO_REQUEST_BITRATE_KBPS ||
              process.env.VIDEO_BITRATE_KBPS ||
              defaultBitrate,
          ) || defaultBitrate,
        ),
      ),
  ...
  private findNaluOfType(frame: Buffer, type: number): Buffer | undefined {
    return this.splitAnnexBNalus(frame).find(
      (nalu) => ((nalu[0] || 0) & 0x1f) === type,
    )
  }

  private ensureDecodableH264Frame(
    frame: Buffer,
    sps?: Buffer,
    pps?: Buffer,
  ): Buffer {
  ...
  private isStructurallyUsableLiveFrame(
    cachedFrame: LatestLiveFrame,
  ): { ok: true; frame: Buffer } | { ok: false; reason: string } {
    const preparedFrame = this.ensureDecodableH264Frame(
      cachedFrame.frame,
      cachedFrame.sps,
      cachedFrame.pps,
    )
  ...
  attachAlertCommandBridge(
    ...
    this.boundRequestScreenshotHandler = ({ vehicleId, channel, alertId }) => {
      ...
      for (const ch of channels) {
        ...
        console.log(
          `Alert ${alertId}: Requesting screenshot from ${vehicleId} channel ${ch}`,
        )
    ...
    this.boundRequestCameraVideoHandler = ({
      vehicleId,
      channel,
      startTime,
      endTime,
      ...
      for (const ch of channels) {
        this.startVideo(vehicleId, ch)
        console.log(
          `Alert ${alertId}: Requesting camera SD card video from ${vehicleId} channel ${ch}`,
        )
  ...
  private detachAlertCommandBridge(): void {
    if (
      this.boundAlertManagerForCommands &&
      this.boundRequestScreenshotHandler
    ) {
      this.boundAlertManagerForCommands.off(
        'request-screenshot',
        this.boundRequestScreenshotHandler,
      )
    }
  ...
  private getAlertEvidenceCheckDelaysMs(): number[] {
    const configured = String(
      process.env.ALERT_EVIDENCE_CHECK_DELAYS_MS || '6000,18000',
    )
      .split(',')
  ...
  private getAlertEvidenceVideoDurationSec(): number {
    return Math.max(
      8,
      Math.min(30, Number(process.env.ALERT_EVIDENCE_VIDEO_DURATION_SEC || 15)),
    )
  }
  ...
  private hasAlertScreenshotEvidence(alert: any): boolean {
    ...
    const screenshotCount = Number(
      evidence?.screenshotCount || screenshots.length || 0,
    )
  ...
  private scheduleAlertEvidenceChecks(alert: any): void {
    ...
    const channels = this.resolveAlertCaptureChannels(vehicleId, channel)
    const timers = this.getAlertEvidenceCheckDelaysMs().map((delayMs) =>
      setTimeout(() => {
        ...
        for (const ch of channels) {
          this.startVideo(vehicleId, ch)
          if (!hasVideo) {
            this.scheduleCameraReportRequests(
              vehicleId,
              ch,
              windowStart,
              windowEnd,
          ...
          if (!hasScreenshot || !hasVideo) {
            void this.requestScreenshotWithFallback(vehicleId, ch, {
              ...
              console.warn(
                `Alert ${alertId}: retry evidence capture failed for ${vehicleId} ch${ch}:`,
                error?.message || error,
              )
            })
          }
  ...
  getVendorAlertTelemetry() {
    const mapToObject = (map: Map<string, number>) =>
      Array.from(map.entries()).reduce<Record<string, number>>(
        (acc, [k, v]) => {
          acc[k] = v
          return acc
        },
    ...
    return {
      ...
      emittedBySourceCode: mapToObject(
        this.vendorAlertTelemetry.emittedBySourceCode,
      ),
      suppressedByReason: mapToObject(
        this.vendorAlertTelemetry.suppressedByReason,
      ),
      parseFailuresByReason: mapToObject(
        this.vendorAlertTelemetry.parseFailuresByReason,
      ),
    }
  }
  ...
  private isStrictAlertMode(): boolean {
    const mode = String(process.env.ALERT_MODE || 'strict')
      .trim()
      .toLowerCase()
    return mode === 'strict'
  }
  ...
  private isVendorAlertEmittable(mapped: VendorMappedAlert): boolean {
    ...
    if (confidence !== 'high') {
      this.bumpVendorTelemetry(
        'suppressedByReason',
        `strict_non_high_confidence_${confidence}`,
      )
      return false
    }
    return true
  ...
  private async processMessage(
    ...
    if (!message) {
      ...
      void this.protocolMessageStorage
        .save({
        ...
        .catch((error) => {
          console.error(
            'Failed to persist raw parse-failed protocol message:',
            error,
          )
        })
    ...
    RawIngestLogger.write('jt808_message', {
      ...
      messageIdHex: `0x${Number(message.messageId || 0)
        .toString(16)
        .padStart(4, '0')}`,
      ...
      bodyTextPreview: this.buildPayloadPreview(
    ...
    switch (message.messageId) {
      ...
      case 0x0001:
        ...
        if (message.body.length >= 5) {
          ...
          console.log(
            `   Reply to: 0x${replyMsgId.toString(16).padStart(4, '0')} (serial ${replySerial})`,
          )
          console.log(
            `   Result: ${result === 0 ? 'Success' : result === 1 ? 'Failure' : result === 2 ? 'Message error' : 'Not supported'}`,
          )

          if (replyMsgId === 0x9101) {
            console.log(
              `   Video stream request acknowledged - waiting for RTP data...`,
            )
      ...
      case 0x1205: // Resource list response
        if (this.verboseResourceLogs) {
          console.log(
            `📝 Resource list response (0x1205) from ${message.terminalPhone}`,
          )
        }

        if (
          message.isSubpackage &&
          message.packetCount &&
          message.packetIndex
        ) {
          ...
          const parsedResourceList = this.parseResourceList(
            message.terminalPhone,
            message.body,
          )
          if (parsedResourceList) {
            this.emitResourceListAlerts(
              message.terminalPhone,
              parsedResourceList,
            )
          }
      ...
      case 0x1206: // File upload completion notification
        console.log(
          `File upload completion (0x1206) from ${message.terminalPhone}`,
        )
        if (message.body.length >= 3) {
          ...
          console.log(
            `   Upload result: ${result === 0 ? 'success' : 'failure'}`,
          )
        }
        break
      ...
      case 0x0801: // Multimedia data upload
        if (message.isSubpackage && this.verboseMediaLogs) {
          console.log(
            `📦 0x0801 subpackage ${message.packetIndex}/${message.packetCount} from ${message.terminalPhone}, body=${message.body.length}`,
          )
        }
  ...
  private hasFreshStreamActivity(vehicleId: string, channel: number): boolean {
    const lastPacketAt = this.lastRtpPacketAt.get(
      this.getVideoStreamKey(vehicleId, channel),
    )
  ...
  private shouldSkipStartVideo(vehicleId: string, channel: number): boolean {
    ...
    if (
      lastRequestedAt &&
      Date.now() - lastRequestedAt < this.startVideoCooldownMs
    ) {
      return true
    }
  ...
  private handleTerminalRegister(message: any, socket: net.Socket): void {
    ...
    console.log(
      `✅ Vehicle registered: ${message.terminalPhone} from ${ipAddress}`,
    )
  ...
  private parseTerminalRegisterInfo(
    body: Buffer,
    terminalPhone: string,
    ipAddress?: string,
    ...
    try {
      ...
      const manufacturerId =
        body.slice(4, 9).toString('ascii').replace(/\0/g, '').trim() ||
        undefined
      const terminalModel =
        this.decodeTerminalText(body.slice(9, 29)) || undefined
      const terminalId =
        this.decodeTerminalText(body.slice(29, 36)) || undefined
  ...
  private emitVendorMappedAlerts(
    ...
    const baseTimestamp =
      options.timestamp ||
      this.lastKnownLocation.get(vehicleId)?.timestamp ||
      new Date()
    const baseLocation =
      options.location ||
      (this.lastKnownLocation.get(vehicleId)
        ? {
    ...
    for (const vendorMapped of mappedList) {
      ...
      void this.alertManager
        .processExternalAlert({
          vehicleId,
          ...
          metadata: {
            ...this.buildAlertContextMetadata(
              vehicleId,
        ...
        .catch((error) => {
          console.error(
            'Failed to process/forward vendor-mapped external alert:',
            error,
          )
        })
    }
  ...
  private handleTerminalAuth(message: any, socket: net.Socket): void {
    ...
    if (!vehicle) {
      ...
      console.log(
        `✅ Camera authenticated: ${message.terminalPhone} from ${ipAddress}`,
      )
      ...
      if (this.videoProcessingEnabled) {
        ...
        setTimeout(() => {
          console.log(
            `🔍 Querying capabilities for ${message.terminalPhone}...`,
          )
          this.queryCapabilities(message.terminalPhone)
        }, 1000)
      }

      const autoConfigureMask =
        String(
          process.env.AUTO_CONFIGURE_VIDEO_ALARM_MASK ?? 'true',
        ).toLowerCase() !== 'false'
      if (autoConfigureMask) {
        const configuredMask =
          Number(process.env.VIDEO_ALARM_MASK_WORD ?? 0) >>> 0
        setTimeout(() => {
          const ok = this.setVideoAlarmMask(
            message.terminalPhone,
            configuredMask,
          )
          if (ok) {
            console.log(
              `Set video alarm mask (0x007A)=0x${configuredMask.toString(16).padStart(8, '0')} for ${message.terminalPhone}`,
            )
          }
        }, 1500)
      }
    ...
    if (vehicle) {
      ...
      const hasDiscoveredChannels =
        Array.isArray(vehicle.channels) && vehicle.channels.length > 0
      if (this.videoProcessingEnabled && !hasDiscoveredChannels) {
        setTimeout(() => {
          console.log(
            `Querying capabilities for ${message.terminalPhone} after auth...`,
          )
          this.queryCapabilities(message.terminalPhone)
        }, 1000)
      }
  ...
  private handleLocationReport(
    ...
    if (message.body.length >= 28) {
      ...
      if (this.verboseLocationLogs) {
        ...
        console.log(
          `Status flags: 0x${statusFlag.toString(16).padStart(8, '0')}`,
        )
      ...
      while (offset < message.body.length - 2) {
        ...
        if (this.verboseLocationLogs) {
          console.log(
            `  ID: 0x${infoId.toString(16).padStart(2, '0')} | Length: ${infoLength} | Data: ${infoData.toString('hex')}`,
          )
          ...
          if (infoId === 0x64)
            console.log(`    → ADAS Extension (vendor/proprietary)`)
          if (infoId === 0x65)
            console.log(`    → DMS Extension (vendor/proprietary)`)
        }
      ...
      if (this.verboseLocationLogs && offset === 28) {
        console.log(
          `  ⚠️  NO ADDITIONAL INFO FIELDS - Cameras not sending alert data`,
        )
      }
    }

    // Parse location and alert data
    const alert = AlertParser.parseLocationReport(
      message.body,
      message.terminalPhone,
    )
    const additionalInfo = this.extractLocationAdditionalInfoFields(
      message.body,
    )
    ...
    if (alert) {
      ...
      const vendorMappedList = this.detectVendorAlarmsFromLocationBody(
        message.body,
      )
      ...
      const bodyVendorMapped = this.detectVendorAlarmsFromPayload(
        message.body,
        'global_payload',
      )
    ...
    this.pushMessageTrace(message, rawFrame, {
      ...
      parsedAlert: alert
        ? {
            ...
            rawAlarmFlagHex:
              typeof alert.rawAlarmFlag === 'number'
                ? `0x${alert.rawAlarmFlag.toString(16).padStart(8, '0')}`
                : null,
            rawStatusFlagHex:
              typeof alert.rawStatusFlag === 'number'
  ...
  private handleLocationBatchUpload(
    message: any,
    socket: net.Socket,
    rawFrame?: Buffer,
  ): void {
    ...
    while (offset + 2 <= body.length) {
      ...
      const itemAdditionalInfo =
        this.extractLocationAdditionalInfoFields(itemBody)
    ...
    console.log(
      `Location batch 0x0704 from ${message.terminalPhone}: declared=${declaredCount}, parsed=${parsed}, uploadType=${uploadType}, alerts=${processedAlerts}`,
    )
    this.pushMessageTrace(message, rawFrame, {
      parser: 'location-batch-0x0704',
      parseSuccess: true,
  ...
  private handleDataUplinkPassThrough(
    message: any,
    socket: net.Socket,
    rawFrame?: Buffer,
  ): void {
  ...
  private handleUnknownMessage(
    ...
    const mappedAlerts = this.detectVendorAlarmsFromPayload(
      body,
      'global_payload',
    )
    this.emitVendorMappedAlerts(message.terminalPhone, mappedAlerts, {
      sourceMessageId: `0x${Number(message.messageId || 0)
        .toString(16)
        .padStart(4, '0')}`,
  ...
  private isAlarmPassThroughType(passThroughType: number): boolean {
    if (
      !Number.isFinite(passThroughType) ||
      passThroughType < 0 ||
      passThroughType > 0xff
    ) {
      return false
    }
  ...
  private extractPassThroughAlarms(
    ...
    if (!canAttemptDecode) {
      this.bumpVendorTelemetry(
        'suppressedByReason',
        `pass_through_type_not_whitelisted_${passThroughType}`,
      )
      return []
    }
    ...
    if (textCode !== null) {
      const mapped = this.mapVendorAlarmCode(textCode, {
        allowPlatformVideoCodes: true,
      })
      if (mapped) {
        add({
          ...mapped,
  ...
  private decodeCompactA1VideoAlarm(
    ...
    const mapped = this.mapVendorAlarmCode(alarmCode, {
      allowPlatformVideoCodes: true,
    })
    ...
    const channel =
      maybeChannel >= 1 && maybeChannel <= 32 ? maybeChannel : undefined
  ...
  private resolveAlertCaptureChannels(
    vehicleId: string,
    requestedChannel?: number,
  ): number[] {
    const vehicle = this.vehicles.get(vehicleId)
    const configuredDefaults = String(
      process.env.ALERT_CAPTURE_CHANNELS || '1,2',
    )
      .split(',')
      .map((value) => Number(String(value || '').trim()))
      .filter(
  ...
  private detectVendorAlarmsFromLocationBody(
    ...
    while (offset + 2 <= body.length) {
      ...
      const mappedList = this.detectVendorAlarmsFromPayload(
        infoData,
        'location_extension',
      )
  ...
  private detectVendorAlarmsFromPayload(
    ...
    for (const frame of peripheralFrames) {
      ...
      if (strictMode) {
        ...
        const userDefinedPeripheral =
          frame.peripheralType >= 0xf0 && frame.peripheralType <= 0xff
        const proprietaryCommand =
          frame.commandType >= 0x44 && frame.commandType <= 0xff
        if (!userDefinedPeripheral || !proprietaryCommand) {
          this.bumpVendorTelemetry(
            'suppressedByReason',
            'strict_peripheral_type_or_command_not_vendor',
          )
          continue
        }
      }
    ...
    if (results.length === 0) {
      this.bumpVendorTelemetry(
        'suppressedByReason',
        'no_deterministic_vendor_pattern_detected',
      )
    }
  ...
  private mapVendorAlarmsFromBytes(
    ...
    const channelMatch = combined.match(
      /\bch(?:annel)?\s*[:#-]?\s*(\d{1,2})\b/i,
    )
    const channel = channelMatch ? Number(channelMatch[1]) : undefined
    ...
    if (requireDeterministic && results.length === 0) {
      this.bumpVendorTelemetry(
        'parseFailuresByReason',
        'no_mapped_vendor_codes',
      )
    }
  ...
  private extractVendorAlarmCodeFromText(text: string): number | null {
    ...
    const match = text.match(
      /\b(1000[1-8]|10016|10017|1010[1-7]|10116|10117|1120[1-3])\b/,
    )
  ...
  private decodePeripheralProtocolFrames(payload: Buffer): Array<{
    ...
    for (const frame of frames) {
      ...
      const chosen =
        parsedCandidates.find((c) => c.validChecksum) || parsedCandidates[0]
  ...
  private processAlert(
    ...
    const hasKnownVideoAlarms = !!(
      alert.videoAlarms &&
      Object.values(alert.videoAlarms).some((v) => v === true)
    )
    ...
    const hasDrivingBehavior =
      alert.drivingBehavior &&
      (alert.drivingBehavior.fatigue ||
        alert.drivingBehavior.phoneCall ||
        alert.drivingBehavior.smoking ||
        alert.drivingBehavior.custom > 0)
    const hasSignalLoss =
      alert.signalLossChannels && alert.signalLossChannels.length > 0
    const hasBlocking =
      alert.blockingChannels && alert.blockingChannels.length > 0
    const hasMemoryFailures =
      alert.memoryFailures &&
      (alert.memoryFailures.main.length > 0 ||
        alert.memoryFailures.backup.length > 0)
    ...
    if (!hasAnyAlert) {
      void this.alertManager
        .processAlert(enriched as LocationAlert)
        .catch((error) => {
          console.error(
            'Failed to process/forward no-alert state packet:',
            error,
          )
        })
      return
    ...
    console.log(
      `Vehicle: ${alert.vehicleId} | Time: ${alert.timestamp.toISOString()}`,
    )
    ...
    if (hasDrivingBehavior) {
      ...
      if (behavior.fatigue) {
        console.log(
          `  😴 FATIGUE - Level: ${behavior.fatigueLevel}/100 ${behavior.fatigueLevel > 70 ? '⚠️ CRITICAL' : ''}`,
        )
      }
    ...
    if (hasBlocking) {
      console.log(
        `🚫 SIGNAL BLOCKING - Channels: ${alert.blockingChannels!.join(', ')}`,
      )
    }
    ...
    if (hasMemoryFailures) {
      ...
      if (alert.memoryFailures!.backup.length) {
        console.log(
          `💾 BACKUP MEMORY FAILURES: ${alert.memoryFailures!.backup.join(', ')}`,
        )
      }
    }
...
📊 Camera Capabilities for ${vehiclePhone}:`)
    console.log(
      `   Audio: encoding=${inputAudioEncoding}, channels=${inputAudioChannels}, rate=${inputAudioSampleRate}`,
    )
    console.log(
      `   Video: encoding=${videoEncoding}, max channels=${maxVideoChannels}`,
    )
    ...
    if (!this.videoProcessingEnabled) {
      console.log(
        `⏭️ Skipping auto-start of video streams for ${vehiclePhone} because VIDEO_PROCESSING_ENABLED=false`,
      )
      return
    }
  ...
  private handleResourceListSubpackage(
    ...
    if (this.verboseResourceLogs) {
      console.log(
        `Resource list subpackage ${packetIndex}/${packetCount} from ${vehicleId} (partLen=${bodyPart.length})`,
      )
    }
    ...
    for (let i = 1; i <= packetCount; i++) {
      ...
      if (!part) {
        if (this.verboseResourceLogs) {
          console.log(
            `Resource list assembly missing part ${i}/${packetCount} for ${vehicleId}`,
          )
        }
        return
      }
    ...
    if (this.verboseResourceLogs) {
      console.log(
        `Resource list merged ${packetCount} packets for ${vehicleId} (len=${merged.length})`,
      )
    }
  ...
  private parseResourceList(
    ...
    if (body.length >= 6 && (body.length - 6) % 28 === 0) {
      ...
      if (this.verboseResourceLogs) {
        console.log(
          `Resource list header: serial=${querySerial}, total=${expectedTotal}`,
        )
      }
      ...
      if (this.verboseResourceLogs) {
        console.log(
          `Resource list body non-standard: len=${body.length}, inferredOffset=${listOffset}`,
        )
      }
    }
    ...
    for (let i = 0; i < itemCount && offset + 28 <= body.length; i++) {
      ...
      const alarmLabels = alarmBits.map((bit) =>
        this.describeResourceAlarmBit(bit),
      )
      ...
      const alarmSummary =
        alarmLabels.length > 0 ? alarmLabels.join(', ') : 'none'
      if (this.verboseResourceLogs) {
        console.log(
          `  File ${i + 1}: Ch${channel} ${startTime} to ${endTime} (${fileSize} bytes, alarm64=${alarmFlag64Hex}, flags=${alarmSummary})`,
        )
      }
      items.push({
        channel,
    ...
    if (
      ...
      if (this.verboseResourceLogs) {
        console.log(
          `Resource list partial parse: parsed=${items.length}, terminalTotal=${expectedTotal}`,
        )
      }
    }
    ...
    for (const item of resourceList.items) {
      for (const bit of item.alarmBits) {
        ...
        if (this.suppressedResourceListSignals.has(signalCode)) {
          if (this.verboseResourceLogs) {
            console.log(
              `Suppressed resource-list alert signal ${signalCode} for ${vehicleId} bit ${bit}`,
            )
          }
          continue
        }
        ...
        const timestamp =
          this.parseResourceTimestamp(item.endTime) ||
          last?.timestamp ||
          new Date()
        void this.alertManager
          .processExternalAlert({
          ...
          .catch((error) => {
            console.error(
              'Failed to process/forward resource-list alert:',
              error,
            )
          })
      }
  ...
  requestScreenshot(vehicleId: string, channel: number = 1): boolean {
    ...
    console.log(
      `Screenshot requested for vehicle ${vehicleId}, channel ${channel} (spec + legacy fallback)`,
    )
    socket.write(command)
  ...
  private rememberPendingScreenshotRequest(
    ...
    const requestedAt =
      options?.requestedAt && !Number.isNaN(options.requestedAt.getTime())
        ? options.requestedAt
        : undefined
    ...
    const exists = pruned.some(
      (item) =>
        ...
        String(item.requestedAt?.toISOString() || '') ===
          String(requestedAt?.toISOString() || ''),
    )
    if (!exists) {
      pruned.push({
        alertId,
  ...
  private rememberRecentNativeScreenshot(
    vehicleId: string,
    channel: number,
    imageId: string,
  ): void {
    this.recentNativeScreenshots.set(
      this.buildScreenshotKey(vehicleId, channel),
      {
        imageId,
        receivedAt: Date.now(),
      },
  ...
  private getRecentNativeScreenshot(
    vehicleId: string,
    channel: number,
    maxAgeMs: number = 6000,
  ): RecentNativeScreenshot | null {
    const entry = this.recentNativeScreenshots.get(
      this.buildScreenshotKey(vehicleId, channel),
    )
    if (!entry) return null
    if (Date.now() - entry.receivedAt > maxAgeMs) {
      this.recentNativeScreenshots.delete(
        this.buildScreenshotKey(vehicleId, channel),
      )
      return null
    }
  ...
  private async waitForRecentNativeScreenshot(
    vehicleId: string,
    channel: number,
    timeoutMs: number,
  ): Promise<RecentNativeScreenshot | null> {
    const deadline = Date.now() + Math.max(0, timeoutMs)
    while (Date.now() <= deadline) {
      const entry = this.getRecentNativeScreenshot(
        vehicleId,
        channel,
        timeoutMs + 1000,
      )
  ...
  private consumePendingScreenshotRequest(
    vehicleId: string,
    channel: number,
  ): PendingScreenshotRequest | undefined {
    const key = vehicleId
    const list = this.pendingScreenshotRequests.get(key)
  ...
  requestScreenshotAt(
    ...
    console.log(
      `Historical screenshot requested for vehicle ${vehicleId}, channel ${channel}, timestamp ${requestedAt.toISOString()} (spec + legacy fallback)`,
    )
    socket.write(command)
    this.startVideo(vehicleId, channel)
    setTimeout(() => {
  ...
  async requestScreenshotWithFallback(
    ...
    if (!fallbackEnabled) {
      return {
        ...
        commandAccepted,
        fallback: { ok: false, reason: 'disabled' },
      }
    }

    let fallback: ScreenshotFallbackResult = {
      ok: false,
    ...
    const retryLiveFrameScreenshot = async (
      ...
      let current: ScreenshotFallbackResult = {
        ok: false,
        reason: 'live frame not attempted',
      }
      for (let attempt = 0; attempt < attempts && !current.ok; attempt++) {
        if (attempt > 0) {
          await new Promise((r) => setTimeout(r, delayMs))
        }
        current = await this.captureScreenshotFromLiveFrame(
          vehicleId,
          channel,
          options?.alertId,
        )
    ...
    const retryHlsScreenshot = async (
      ...
      for (let attempt = 0; attempt < attempts && !current.ok; attempt++) {
        ...
        current = await this.captureScreenshotFromHLS(
          vehicleId,
          channel,
          options?.alertId,
        )
    ...
    if (captureVideoEvidence) {
      let videoBackup = await this.captureVideoEvidenceFromHLS(
        vehicleId,
        channel,
        videoDurationSec,
        options?.alertId,
      ...
      if (!videoBackup.ok && isWarmupReason(videoBackup.reason)) {
        for (let attempt = 0; attempt < 3 && !videoBackup.ok; attempt++) {
          ...
          videoBackup = await this.captureVideoEvidenceFromHLS(
            vehicleId,
            channel,
            videoDurationSec,
            options?.alertId,
    ...
    if (!commandAccepted && !fallback.ok) {
      return {
        ...
        fallback: {
          ...fallback,
          reason: fallback.reason || 'vehicle not connected',
        },
      }
    }
  ...
  async saveLiveFrameScreenshot(
    ...
    const retryDelayMs = Math.max(
      100,
      Math.min(2000, Number(options?.retryDelayMs) || 400),
    )
    const initialDelayMs = Math.max(
      0,
      Math.min(4000, Number(options?.initialDelayMs) || 1200),
    )
    const timeoutMs = Math.max(
      ...
      Math.min(
        ...
        Number(options?.timeoutMs) ||
          initialDelayMs + retries * retryDelayMs + 4000,
      ),
    )
    ...
    let result: ScreenshotFallbackResult = {
      ok: false,
      reason: 'live frame not attempted',
    }
  ...
  async saveActiveStreamScreenshot(
    ...
    const retryDelayMs = Math.max(
      100,
      Math.min(3000, Number(options?.retryDelayMs) || 600),
    )
    const initialDelayMs = Math.max(
      0,
      Math.min(8000, Number(options?.initialDelayMs) || 1000),
    )
    const timeoutMs = Math.max(
      initialDelayMs + retryDelayMs,
      Math.min(
    ...
    let result: ScreenshotFallbackResult = {
      ok: false,
      reason: 'stream output not attempted',
    }
    let attempt = 0
    ...
    while (Date.now() <= deadline) {
      ...
      if (shouldTryHls) {
        const hlsResult = await this.captureScreenshotFromHLS(
          vehicleId,
          channel,
        )
        if (hlsResult.ok) {
    ...
    try {
        ...
        ffmpeg.on('close', (code) => {
          ...
          if (code === 0 && fs.existsSync(outPath)) {
            ...
            if (stderr) {
              if (
                this.shouldLogNoisy(
                  `video_fallback_ffmpeg:${vehicleId}:${channel}`,
                  10000,
                )
              ) {
                console.warn(
                  `Video evidence fallback ffmpeg stderr: ${stderr.slice(0, 300)}`,
                )
              }
            }
            resolve(false)
      ...
      if (alertId) {
        await this.alertManager
          ...
          .catch((err: any) => {
            console.warn(
              `Failed to register fallback video evidence for ${alertId}:`,
              err?.message || err,
            )
          })
      }
      ...
      return {
        ok: false,
        reason: error?.message || 'video evidence fallback error',
      }
    }
  }
  ...
  private async captureScreenshotFromVideoFile(
    ...
    try {
      ...
      const imageId = await this.imageStorage.saveImage(
        vehicleId,
        channel,
        imageData,
        alertId,
      ...
      if (alertId) {
        await this.alertManager
          .registerAlertScreenshotEvidence({
            alertId,
          ...
          .catch((err: any) => {
            console.warn(
              `Failed to register video-frame screenshot evidence for ${alertId}:`,
              err?.message || err,
            )
          })
      }
      console.log(
        `Alert ${alertId || 'manual'}: Video-frame fallback screenshot saved for ${vehicleId} ch${channel}`,
      )
      return { ok: true, imageId }
    } catch (error: any) {
      return {
        ok: false,
        reason: error?.message || 'video frame screenshot fallback error',
      }
    }
  }
  ...
  private async captureScreenshotFromLiveFrame(
    ...
    try {
      ...
      if (
        Date.now() - cachedFrame.receivedAt >
        this.liveFrameScreenshotFreshnessMs
      ) {
      ...
      const imageData = await new Promise<Buffer | null>((resolve) => {
        ...
        ffmpeg.on('close', (code) => {
          ...
          if (
            code === 0 &&
            chunks.length > 0 &&
            !/corrupt|error|invalid/i.test(stderr)
          ) {
            finish(Buffer.concat(chunks))
          } else {
            finish(null)
      ...
      if (alertId) {
        await this.alertManager
          ...
          .catch((err: any) => {
            console.warn(
              `Failed to register live-frame screenshot evidence for ${alertId}:`,
              err?.message || err,
            )
          })
      }
      console.log(
        `Alert ${alertId || 'manual'}: Live-frame screenshot saved for ${vehicleId} ch${channel}`,
      )
      return { ok: true, imageId }
    } catch (error: any) {
      return {
        ok: false,
        reason: error?.message || 'live frame screenshot fallback error',
      }
    }
  }

  private async captureScreenshotFromHLS(
    vehicleId: string,
    channel: number,
    alertId?: string,
  ): Promise<ScreenshotFallbackResult> {
    try {
      ...
      if (!readiness.ok) {
        console.log(
          `HLS fallback skipped for ${vehicleId} ch${channel}: ${readiness.reason}`,
        )
        return { ok: false, reason: readiness.reason }
      }
      ...
      if (alertId) {
        await this.alertManager
          ...
          .catch((err: any) => {
            console.warn(
              `Failed to register HLS screenshot evidence for ${alertId}:`,
              err?.message || err,
            )
          })
      }
      console.log(
        `Alert ${alertId || 'manual'}: HLS fallback screenshot saved for ${vehicleId} ch${channel}`,
      )
      return { ok: true, imageId }
    } catch (error: any) {
      console.error(
        `HLS fallback screenshot error for ${vehicleId} ch${channel}:`,
        error?.message || error,
      )
  ...
  private getReadyHlsPlaylistState(
    playlistPath: string,
    ...
    try {
      ...
      return {
        ok: false,
        reason: error?.message || 'HLS readiness check failed',
      }
    }
  }
  ...
  private enqueuePendingCameraRequest(req: PendingCameraRequest): void {
    ...
    const exists = list.some(
      (r) =>
        `${r.channel}|${r.startTime}|${r.endTime}|${r.queryResources ? 1 : 0}|${r.requestDownload ? 1 : 0}` ===
        dedupeKey,
    )
    if (exists) return
    list.push(req)
  ...
  private flushPendingCameraRequests(vehicleId: string): void {
    ...
    for (const req of list) {
      ...
      if (req.queryResources)
        this.queryResourceList(req.vehicleId, req.channel, start, end)
      this.requestCameraVideo(req.vehicleId, req.channel, start, end)
      if (req.requestDownload)
        this.requestCameraVideoDownload(req.vehicleId, req.channel, start, end)
    }
  }
  ...
  scheduleCameraReportRequests(
    ...
    if (this.isVehicleConnected(vehicleId)) {
      const querySent = queryResources
        ? this.queryResourceList(vehicleId, channel, startTime, endTime)
        : false
      const requested = this.requestCameraVideo(
        vehicleId,
        channel,
        startTime,
        endTime,
      )
      const downloadSent = requestDownload
        ? this.requestCameraVideoDownload(
            vehicleId,
            channel,
            startTime,
            endTime,
  ...
  requestCameraVideo(
    ...
    if (this.verboseResourceLogs) {
      console.log(
        `🎥 Camera video requested: ${vehicleId} ch${channel} from ${startTime.toISOString()} to ${endTime.toISOString()}`,
      )
    }
    socket.write(command)
    this.pushOutboundMessageTrace(
      vehicleId,
      0x9201,
      serial,
      command,
  ...
  requestCameraVideoDownload(
    ...
    if (this.shouldLogNoisy('ftp_disabled_video_download', 60000)) {
      console.log(
        `FTP download disabled; skipping 0x9206 request for ${vehicleId} ch${channel}.`,
      )
    }
    return false
  }
  ...
  queryResourceList(
    ...
    if (this.verboseResourceLogs) {
      console.log(
        `📝 Query resource list: ${vehicleId} ch${channel} from ${startTime.toISOString()} to ${endTime.toISOString()}`,
      )
    }
    socket.write(command)
    this.pushOutboundMessageTrace(
  ...
  startVideo(vehicleId: string, channel: number = 1): boolean {
    console.log(
      `🎬 startVideo called: vehicleId=${vehicleId}, channel=${channel}`,
    )
    ...
    if (!vehicle || !socket || !vehicle.connected) {
      console.log(
        `  ❌ Cannot start video: vehicle=${!!vehicle}, socket=${!!socket}, connected=${vehicle?.connected}`,
      )
      return false
    }
    ...
    if (this.shouldSkipStartVideo(vehicleId, channel)) {
      if (
        this.shouldLogNoisy(`start_video_skip:${vehicleId}:${channel}`, 10000)
      ) {
        console.log(
          `Skipping 0x9101 for ${vehicleId} channel ${channel} because the stream is already active or was just requested.`,
        )
      }
    ...
    console.log(
      `📡 Sending 0x9101: ServerIP=${serverIp}, TCP=${this.port}, UDP=${this.udpPort}, Channel=${channel}, Stream=${preferredStreamType === 0 ? 'MAIN' : 'SUB'}`,
    )
    socket.write(command)
    this.lastStartVideoAt.set(
      this.getVideoStreamKey(vehicleId, channel),
      Date.now(),
    )
    this.pushOutboundMessageTrace(
  ...
  optimizeVideoParameters(vehicleId: string, channel: number = 1): boolean {
    ...
    const { resolution, frameRate, bitrate } =
      this.getOptimizedVideoParameters()
    ...
    console.log(
      `⚡ Optimizing camera: ${vehicleId} ch${channel} -> res=${resolution}, fps=${frameRate}, bitrate=${bitrate}kbps`,
    )
    socket.write(command)
    return true
  }
  ...
  switchStream(vehicleId: string, channel: number, streamType: 0 | 1): boolean {
    ...
    console.log(
      `🔄 Switching to ${streamType === 0 ? 'MAIN' : 'SUB'} stream: ${vehicleId} channel ${channel}`,
    )
    socket.write(command)
    this.pushOutboundMessageTrace(
      vehicleId,
  ...
  getLiveFrameDebugStatus(
    ...
    return {
      ...
      lastRtpPacketAt: lastPacketAtMs
        ? new Date(lastPacketAtMs).toISOString()
        : null,
      hasCachedKeyframe: !!cachedFrame,
      cachedKeyframeAt: cachedFrame
        ? new Date(cachedFrame.receivedAt).toISOString()
        : null,
  ...
  getLatestResourceList(
    vehicleId: string,
  ): { receivedAt: number; items: ResourceVideoItem[] } | undefined {
  ...
  getRecentMessageTraces(options?: {
    ...
    const messageId =
      typeof options?.messageId === 'number' ? options?.messageId : undefined
    const direction = options?.direction
    const limit = Math.max(
      1,
      Math.min(Number(options?.limit || 50), this.maxMessageTraceBuffer),
    )
  ...
  private pushMessageTrace(
    ...
    if (this.recentMessageTraces.length > this.maxMessageTraceBuffer) {
      const overflow =
        this.recentMessageTraces.length - this.maxMessageTraceBuffer
      this.recentMessageTraces.splice(0, overflow)
    }

    RawIngestLogger.write(
      'jt808_message_trace',
    ...
    try {
      for (const callback of this.messageTraceCallbacks) {
        callback(trace)
      }
    } catch {
  ...
  private pushOutboundMessageTrace(
    ...
    const trace: MessageTraceEntry = {
      ...
      messageIdHex: `0x${Number(messageId || 0)
        .toString(16)
        .padStart(4, '0')}`,
    ...
    if (message.body.length >= 8) {
      ...
      if (mapped) {
        ...
        void this.alertManager
          ...
          .catch((error) => {
            console.error(
              'Failed to process/forward multimedia-event alert:',
              error,
            )
          })
      }
      ...
      const last = this.lastKnownLocation.get(message.terminalPhone)
      const vendorMapped = this.detectVendorAlarmsFromPayload(
        message.body,
        'global_payload',
      )
  ...
  private mapMultimediaEvent(
    eventCode: number,
  ): { type: string; priority: AlertPriority } | null {
    ...
    if (eventCode === 3) {
      return {
        type: 'Collision/Rollover Trigger',
        priority: AlertPriority.CRITICAL,
      }
    }
    if (eventCode === 4) {
    ...
    if (eventCode === 6) {
      return {
        type: 'Door Transition Speed Event',
        priority: AlertPriority.MEDIUM,
      }
    }
    if (eventCode === 7) {
  ...
  private extractKeywordAlertFromBinary(payload: Buffer): {
    ...
    const codeMap: Record<
      number,
      { type: string; priority: AlertPriority; signalCode: string }
    > = {
      0x0101: {
  ...
  private extractKeywordAlert(text: string): {
    ...
    const patterns: Array<{
      ...
      {
        re: /\b(panic|sos|emergency)\b/i,
        type: 'Emergency Alarm',
        priority: AlertPriority.CRITICAL,
        signalCode: 'custom_keyword_emergency',
      },
      {
        re: /\b(collision|rollover|crash|accident)\b/i,
        type: 'Collision/Accident',
        priority: AlertPriority.CRITICAL,
        signalCode: 'custom_keyword_collision',
      },
      {
        re: /\b(fatigue|yawn|drowsy|sleepy)\b/i,
        type: 'Fatigue Alert',
        priority: AlertPriority.HIGH,
        signalCode: 'custom_keyword_fatigue',
      },
      {
        re: /\b(seat\s*belt|seatbelt|seatbelt\s*detected|unbelted|no\s*seat\s*belt|without\s*seat\s*belt)\b/i,
        type: 'Seat-Belt Detection',
        priority: AlertPriority.HIGH,
        signalCode: 'custom_keyword_no_seatbelt',
      },
      {
        re: /\b(smok|cigarette)\b/i,
        type: 'Smoking',
        priority: AlertPriority.HIGH,
        signalCode: 'custom_keyword_smoking',
      },
      {
        re: /\b(phone|cellphone|mobile)\b/i,
        type: 'Phone Calling',
        priority: AlertPriority.HIGH,
        signalCode: 'custom_keyword_phone',
      },
      {
        re: /\b(speed|overspeed)\b/i,
        type: 'Overspeed Alert',
        priority: AlertPriority.HIGH,
        signalCode: 'custom_keyword_speed',
      },
      {
        re: /\b(camera).*(covered|blocked|mask|obstruct)/i,
        type: 'Camera Blocked',
        priority: AlertPriority.HIGH,
        signalCode: 'custom_keyword_camera_covered',
      },
      {
        re: /\b(storage|memory).*(fail|error|fault)/i,
        type: 'Storage Failure',
        priority: AlertPriority.HIGH,
        signalCode: 'custom_keyword_storage_failure',
      },
      {
        re: /\b(gnss|gps).*(antenna).*(disconnect|fault|error)/i,
        type: 'GNSS Antenna Issue',
        priority: AlertPriority.MEDIUM,
        signalCode: 'custom_keyword_gnss_antenna',
      },
      {
        re: /\b(alert|alarm|warning|violation)\b/i,
        type: 'Custom Alert',
        priority: AlertPriority.MEDIUM,
        signalCode: 'custom_keyword_alert',
      },
    ]
  ...
  private async handleMultimediaData(
    message: any,
    socket: net.Socket,
  ): Promise<void> {
    try {
      const multimedia = MultimediaParser.parseMultimediaData(
        message.body,
        message.terminalPhone,
      )

      if (multimedia && multimedia.type === 'jpeg') {
        const pending = this.consumePendingScreenshotRequest(
          message.terminalPhone,
          multimedia.channel,
        )
        ...
        if (!this.isAcceptableNativeScreenshot(multimedia.data)) {
          console.warn(
            `Rejected low-quality native screenshot from ${message.terminalPhone} channel ${multimedia.channel} (${multimedia.data.length} bytes)`,
          )
        } else {
          ...
          if (imageId) {
            this.rememberRecentNativeScreenshot(
              message.terminalPhone,
              multimedia.channel,
              String(imageId),
            )
          ...
          if (pendingAlertId && imageId) {
            await this.alertManager
              ...
              .catch((err: any) => {
                console.warn(
                  `Failed to register multimedia screenshot evidence for ${pendingAlertId}:`,
                  err?.message || err,
                )
              })
          }
          ...
          if (imageId) {
            console.log(
              `Saved native image from ${message.terminalPhone} channel ${multimedia.channel}`,
            )
          }
        }
      }
