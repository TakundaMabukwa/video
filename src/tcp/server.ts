...
export class JTT808Server {
  ...
  private mapVendorAlertText(
    ...
    const patterns: Array<{ re: RegExp; type: string; priority: AlertPriority; signalCode: string }> = [
      ...
      { re: /\bfollowing\s+distance\s+too\s+close\b/i, type: 'ADAS: Too Close', priority: AlertPriority.HIGH, signalCode: 'adas_10003_following_distance_too_close' },
      { re: /\bpedestrian\s+collision\s+alarm\b/i, type: 'ADAS: Pedestrian Collision', priority: AlertPriority.CRITICAL, signalCode: 'adas_10004_pedestrian_collision_alarm' },
      { re: /\bfrequent\s+lane\s+change\s+alarm\b/i, type: 'ADAS: Frequent Lane Change Alert', priority: AlertPriority.HIGH, signalCode: 'adas_10005_frequent_lane_change_alarm' },
      { re: /\broad\s+sign\s+over-?limit\s+alarm\b/i, type: 'ADAS: Road Sign Exceedance Alert', priority: AlertPriority.MEDIUM, signalCode: 'adas_10006_road_sign_over_limit_alarm' },
      { re: /\bobstruction\s+alarm\b/i, type: 'ADAS: Obstacle Alert', priority: AlertPriority.MEDIUM, signalCode: 'adas_10007_obstruction_alarm' },
      ...
      { re: /\broad\s+sign\s+identification\s+event\b/i, type: 'ADAS: Road Sign Recognition Event', priority: AlertPriority.LOW, signalCode: 'adas_10016_road_sign_identification_event' },
...
📊 Camera Capabilities for ${vehiclePhone}:`);
  ...
  private extractKeywordAlert(text: string): {
    ...
    const patterns: Array<{ re: RegExp; type: string; priority: AlertPriority; signalCode: string }> = [
      ...
      { re: /\b(fatigue|yawn|drowsy|sleepy)\b/i, type: 'Fatigue Alert', priority: AlertPriority.HIGH, signalCode: 'custom_keyword_fatigue' },
      { re: /\b(seat\s*belt|seatbelt|seatbelt\s*detected|unbelted|no\s*seat\s*belt|without\s*seat\s*belt)\b/i, type: 'Seat-Belt Detection', priority: AlertPriority.HIGH, signalCode: 'custom_keyword_no_seatbelt' },
      { re: /\b(smok|cigarette)\b/i, type: 'Smoking', priority: AlertPriority.HIGH, signalCode: 'custom_keyword_smoking' },
      { re: /\b(phone|cellphone|mobile)\b/i, type: 'Phone Calling', priority: AlertPriority.HIGH, signalCode: 'custom_keyword_phone' },
      { re: /\b(speed|overspeed)\b/i, type: 'Overspeed Alert', priority: AlertPriority.HIGH, signalCode: 'custom_keyword_speed' },
      { re: /\b(camera).*(covered|blocked|mask|obstruct)/i, type: 'Camera Blocked', priority: AlertPriority.HIGH, signalCode: 'custom_keyword_camera_covered' },
