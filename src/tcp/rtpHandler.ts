...
export class TCPRTPHandler {
  ...
  private vehicleIngressStats = new Map<
    string,
    {
  ...
  setRawPacketCallback(
    callback: (vehicleId: string, channel: number, packet: Buffer) => void,
  ): void {
    this.rawPacketCallback = callback
  }
  ...
  handleRTPPacket(buffer: Buffer, vehicleId: string): void {
    ...
    ingressStats.lastBufferPrefixHex = buffer
      .slice(0, Math.min(buffer.length, 16))
      .toString('hex')
    ...
    if (this.rawPacketCallback) {
      try {
        this.rawPacketCallback(vehicleId, header.channelNumber, buffer)
      } catch (error) {
        console.error('Raw packet callback error:', error)
      }
    }
    ...
    if (dataType === 0x04) {
      this.handleTransparentAlertPayload(
        vehicleId,
    ...
    if (this.frameCount === 1) {
      console.log(
        `[RTP] First video frame from ${vehicleId} channel ${header.channelNumber}`,
      )
    }
  }

  private handleTransparentAlertPayload(
    vehicleId: string,
    channel: number,
    payload: Buffer,
  ): void {
  ...
  private mapVendorAlertText(
    ...
    const patterns: Array<{
      ...
      {
        re: /\bseat\s*belt\s*detected\b/i,
        type: 'Seat-Belt Detection',
        priority: AlertPriority.MEDIUM,
        signalCode: 'vendor_seatbelt_detected',
      },
      {
        re: /\bno\s+driver\s+detected\b/i,
        type: 'No Driver Detected',
        priority: AlertPriority.HIGH,
        signalCode: 'vendor_no_driver_detected',
      },
      {
        re: /\bcamera\s+(obstructed|blocked|covered)\b/i,
        type: 'Camera Blocked',
        priority: AlertPriority.HIGH,
        signalCode: 'vendor_camera_obstructed',
      },
      {
        re: /\bidle\s+alert\b/i,
        type: 'Idle Alert',
        priority: AlertPriority.MEDIUM,
        signalCode: 'vendor_idle_alert',
      },
      {
    ...
    return hit
      ? { type: hit.type, priority: hit.priority, signalCode: hit.signalCode }
      : null
  }

  private mapVendorCode(
    code: number,
  ): { type: string; priority: AlertPriority; signalCode: string } | null {
    const map: Record<
      number,
